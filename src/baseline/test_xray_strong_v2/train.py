"""Strong X-ray-only ResNet50 experiment - version 2."""
...
from .model import MaxStrengthXrayResNet50
...
# ---------------------------------------------------------------------
# Move nested image lists to device
# ---------------------------------------------------------------------

def move_radiographs_to_device(
    radiographs,
    device,
):
    """
    Move the nested per-sample radiograph lists to the target device.

    Dataset structure:

        batch["radiographs"]
            -> sample 1 -> [image_1, image_2, ...]
            -> sample 2 -> [image_1, image_2, ...]

    The model expects this nested Python-list structure.
    """

    moved = []

    for image_list in radiographs:

        if len(image_list) == 0:
            raise RuntimeError(
                "Encountered a sample with zero radiographs."
            )
...
def horizontally_flip_radiographs(
    radiographs,
):
    """
    Create horizontally flipped copies for test-time augmentation.

    Images use [C, H, W] layout, therefore horizontal flipping
...
def get_training_labels(dataset):
    """
    Get training labels without unnecessarily loading images.
    """
...
def compute_pos_weights(labels):
    """
    Compute independent positive-class weights.
...
def build_model():
    return MaxStrengthXrayResNet50(
...
def train_one_epoch(
    ...
    # model.train() changes BatchNorm back to train mode,
    # therefore freeze the BatchNorm statistics afterwards.
...
def probabilities_to_logits(
    ...
    Used only after probability-level TTA.
    """
...
def main():
    ...
    # -------------------------------------------------------------
    # Test with validation-derived thresholds
    # -------------------------------------------------------------
